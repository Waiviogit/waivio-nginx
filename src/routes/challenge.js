...
<body>
    ...
    <script>
        ...
        function onCaptchaSuccess(token) {
            ...
            .then(response => {
                // Handle redirect (302, 301, or status 0 for opaque redirects)
                if (response.status === 302 || response.status === 301 || response.status === 0) {
                    const location = response.headers.get('Location');
                    if (location) {
                        // Resolve relative URLs
                        const redirectUrl = location.startsWith('http') ? location : new URL(location, window.location.origin).href;
